dir="xperiments_results/tpch"
# dir="."
mkdir -p $dir/processed
cat template.csv > $dir/raw_data_min_max.csv
cat template.csv > $dir/raw_data_cluster.csv
cat template.csv > $dir/raw_data_bloom.csv
cat template.csv > $dir/raw_data_dictionary.csv
rm test.db
duckdb -f sql_queries_tpch.sql > $dir/output_correct.txt
rm test.db
./build/release/duckdb_min_max -f sql_queries_tpch.sql 2>> $dir/raw_data_min_max.csv > $dir/output_min_max.txt
rm test.db
./build/release/duckdb_cluster -f sql_queries_tpch.sql 2>> $dir/raw_data_cluster.csv > $dir/output_cluster.txt
rm test.db
./build/release/duckdb_bloom -f sql_queries_tpch.sql 2>> $dir/raw_data_bloom.csv > $dir/output_bloom.txt
rm test.db
./build/release/duckdb_dictionary -f sql_queries_tpch.sql 2>> $dir/raw_data_dictionary.csv > $dir/output_dictionary.txt
rm test.db
python3 data_processor.py 8 22 0 $dir/raw_data_min_max.csv $dir/raw_data_cluster.csv $dir/raw_data_bloom.csv $dir/raw_data_dictionary.csv
rm test.db
mv pruning.csv $dir/processed/
mv query_time.csv $dir/processed/
mv size.csv $dir/processed/
mv ingestion.csv $dir/processed/