...
QUERIES_START = NUM_TABLES+NUM_QUERIES+2
...
duckdb.execute("CREATE TABLE Result_size (Table_name VARCHAR, min_max_rows_scanned FLOAT, cluster_rows_scanned FLOAT, bloom_rows_scanned FLOAT);")
duckdb.execute("CREATE TABLE Result_ingestion (Table_name VARCHAR, min_max_rows_scanned FLOAT, cluster_rows_scanned FLOAT, bloom_rows_scanned FLOAT);")
for idx,in_file in enumerate(in_files):
    ...
    for i in range(NUM_TABLES):
        sizes[idx].append(duckdb.execute(f"SELECT COALESCE(SUM(CAST(Data->'size' AS INTEGER)),0) FROM tbl WHERE type = 'END_INITIALISE_ADDITIONAL_STATS' AND CommandID = {i};").fetchone()[0])
        ingestion_times[idx].append(duckdb.execute(f"SELECT ROUND((end_time-start_time)/1000000000.0,3) FROM times WHERE CommandID = {i}").fetchone()[0])
        tables[idx].append(duckdb.execute(f"SELECT CAST(Data->'command' AS VARCHAR) FROM tbl WHERE type='SQL_COMMAND_RUN_START' AND CommandID = {i}").fetchone()[0].split()[2])
    ...
    for i in range(NUM_QUERIES):
        ...
        rows_scanned[idx].append(duckdb.execute(f"SELECT ROUND(SUM(CAST(Data->'count' AS INTEGER))/1000000.0,2) AS val FROM tbl WHERE type = 'SCANNED_ROWS' and CommandID = {NUM_TABLES+1+i};").fetchone()[0])
